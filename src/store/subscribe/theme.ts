...
function addThemeCssVarsToHtml(themeVars: ThemeVars) {
  const keys: string[] = Object.keys(themeVars) as string[];
  const style: string[] = [];
  keys.forEach(key => {
    ...
    if (color) {
      ...
      if (key === 'colorPrimary') {
        const newKey = 'primaryColor';
        style.push(`--${kebabCase(newKey)}: ${r},${g},${b}`);
        const colorPalettes = getColorPalettes(color);

        colorPalettes.forEach((palette, index) => {
          const { r: pR, g: pG, b: pB } = getRgbOfColor(palette);
          style.push(`--${kebabCase(newKey)}${index + 1}: ${pR},${pG},${pB}`);
        });
      }
    }
  });
