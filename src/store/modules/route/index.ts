...
import { fetchUserRoutes, fetchUserMenu } from '@/service';
import {
  localStg,
...
export const useRouteStore = defineStore('route-store', {
  ...
  actions: {
    ...
    async handleAuthRoute(routes: AuthRoute.Route[]) {
      ...
      (this.menus as App.GlobalMenuOption[]).push(...transformBackedMenuToMenu(menus.data!));
    ...
    async initDynamicRoute() {
      ...
      if (!error) {
        ...
        await this.handleAuthRoute(sortRoutes(data.routes));
    ...
    async initStaticRoute() {
      ...
      const routes = filterAuthRoutesByUserPermission(staticRoutes, Object.keys(auth.grantedPolicies));
      await this.handleAuthRoute(routes);
