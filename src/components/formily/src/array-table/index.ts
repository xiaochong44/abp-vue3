...
const useArrayTableSources = (
  ...
  const parseSources = (schema: Schema): ObservableColumnSource[] | undefined => {
    if (
      ...
      const columnProps =
        (field?.component && Array.isArray(field.component) && field?.component[1]) || schema['x-component-props'] || {}
...
const useArrayTableColumns = (
  ...
  return sources.reduce((buf: any, { name, columnProps, schema, display }, key) => {
        ...
        return wrapSSR(
          h(
            Select,
            {
              ...
              class: [
                cls(`${prefixCls}-status-select`, hashId, {
    ...
    return () => {
      ...
      const renderPagination = () => {
        ...
        return h(
          'div',
          {
            class: [cls(`${prefixCls}-pagination`, hashId)],
          },
          h(Space, {}, () => [
            h(StatusSelect, {
              value: current.value,
  ...
  defineComponent<TableProps>({
    ...
    setup(_props, { attrs, slots }) {
      ...
      return () => {
        ...
        if (!pagination) {
          return renderTable(dataSource, undefined)
        }
