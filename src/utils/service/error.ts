 ...
 * @param axiosError - 错误
 */
export function handleAxiosError(axiosError: AxiosError<any>) {
  ...
  const actions: Common.StrategyAction[] = [
    ...
    [
      // 请求不成功的错误
      axiosError.response?.status !== 200 && axiosError.response?.status !== 204,
      () => {
        const errorCode: ErrorStatus = (axiosError.response?.status as ErrorStatus) || 'DEFAULT';
        // rest服务返回的错误信息
        let errorMsg = axiosError.response?.data?.error_description || axiosError.response?.data?.error;
        if (typeof errorMsg === 'object') {
          errorMsg = errorMsg.message;
        }
        const msg = ERROR_STATUS[errorCode];
        Object.assign(error, { code: errorCode, msg: errorMsg || msg });
      }
    ]
  ];
