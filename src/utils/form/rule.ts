...
export const createAntRequiredFormRule = (message = '不能为空'): Rule => ({ required: true, message });
...
export const formRules: CustomFormRules = {
  phone: [
    createAntRequiredFormRule('请输入手机号码'),
    { pattern: REGEXP_PHONE, message: '手机号码格式错误', trigger: 'blur' }
  ],
  pwd: [
    createAntRequiredFormRule('请输入密码'),
    { pattern: REGEXP_PWD, message: '密码为6-18位数字/字符/符号，至少2种组合', trigger: 'blur' }
  ],
  code: [
    createAntRequiredFormRule('请输入验证码'),
    { pattern: REGEXP_CODE_SIX, message: '验证码格式错误', trigger: 'blur' }
  ],
